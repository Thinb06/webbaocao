...
function showCustomToast(message, type = 'success') {
    // Tự tạo container nếu trang chưa có (để toast hiện được trên mọi trang)
...
function updateCartBadge() {
    ...
    // Cập nhật tất cả badge có thể có trên các trang khác nhau
    const $badges = $('#cart-count-badge, #cart-count-badge-mobile, #cart-count, .cart-badge');
...
function applyPromoCode() {
    ...
    if (code === 'THEMAN50') {
        activeCoupon = { code: 'THEMAN50', type: 'amount', value: 50000 };
        $msg.text("Áp dụng mã THEMAN50 thành công: Giảm 50.000đ").removeClass('text-danger').addClass('text-success');
...
function submitOrder(event) {
    ...
    const orderId = `THEMAN-${Math.floor(1000 + Math.random() * 9000)}`;
    ...
    if (payment === 'BANK') {
        const bankNoteHtml = `
            <div id="bank-info-container" class="alert alert-info text-start small mb-3" style="font-size: 11.5px; line-height: 1.5;">
                ...
                Chủ TK: <strong>CONG TY THOI TRANG THEMAN</strong><br>
